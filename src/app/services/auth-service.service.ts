  ...
  return new Promise<any>((resolve, reject) => {
    ...
    //     if(user){
      ...
      // });
        // }
        // else{
        //   this.toastr.warning('No records found with this mobile number!')
        // }
    })
    })
...
signUpWithPhoneNumber(recaptchaVerifier, phoneNumber, user) {
  return new Promise<any>((resolve, reject) => {
...
public async enterVerificationCode(code) {
  return new Promise<any>((resolve, reject) => {
    this.confirmationResult.confirm(code).then(async (result) => {
      ...
      this.afs.collection("users").ref.where("mobile", "==", user.phoneNumber).onSnapshot(snap =>{
        snap.forEach(userRef => {
           this.userData = userRef.data()
          console.log("userRef", userRef.data());
          this.localUserService.setUser(this.userData);
...
public async enterSignUpVerificationCode(code) {
  return new Promise<any>((resolve, reject) => {
    this.confirmationResult.confirm(code).then(async (result) => {
      const user = result.user;
